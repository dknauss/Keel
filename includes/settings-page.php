	...
	$screen->add_help_tab(
		array(
			...
			'content' =>
				...
				'<p>' . wp_kses(
					__( '<strong>The environment indicator is the colour-coded label in the admin bar naming the current environment.</strong> It is off by default and worth turning on anywhere somebody might have production and staging open in adjacent browser tabs. Below 960px the label collapses to its icon to save room, but it stays readable to screen readers. It is the opt-in half of this pair on purpose: suppressing mail is silent on production and only acts where it is needed, while the indicator would be visible on every screen of every install and useful mainly where there is more than one site to confuse.', 'keel-defaults' ),
					array( 'strong' => array() )
				) . '</p>' .
				'<p>' . wp_kses(
